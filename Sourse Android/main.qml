...
Window
{
    ...
    property real defaultSize: 2000
    ...
    Repeater // что то вроде цикла, repeat object
    {
        ...
        Rectangle
        {
            ...
            SensorGesture // Сенсоры
            {
                id: sensorGesture
                enabled: true

                gestures : ["QtSensors.whip", "QtSensors.twist"]

                onDetected:
                {
                    if (gesture == "whip")
                    {
                        photoFrame.x = Math.random() * (root.width - defaultSize);
                        photoFrame.y = Math.random() * (root.height - defaultSize);
                    }
                }
            }
            ...
            Image
            {
                ...
                scale: defaultSize / (Math.max(sourceSize.width, sourceSize.height) * folderModel.count); // размер/масштаб
